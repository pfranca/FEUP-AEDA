...
/*
bool Servico::operator< (const Servico& s1, const Servico& s2) const
{
	if (s1.ano < s2.ano)
		return true;
	else if (s1.mes < s2.mes)
		return true;
	else if (s1.dia < s2.dia)
		return true;
	else return false;
}*/

void Servico::imprimeServico(Servico s)
{
	cout << s.getOrigem() << " " << s.getDestino() << " " << s.getData() << " " << endl;
}